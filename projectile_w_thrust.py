...
class Config:
    ...
    thrust = 4000.0      # Newtons (Sustainer Motor)
    burn_time = 15.0     # Seconds (Motor burn duration)
...
class Projectile:
    def __init__(self):
        self.pos = np.array([0.0, 2000.0, 0.0])  
        self.vel = np.array([680.0, 0.0, 0.0])   
        self.aero = AeroDatabase()

    # ### CHANGE 1: We now accept 't' (current time) as an input
    def update(self, t, dt, target_pos):
        # A. ATMOSPHERE
        h = self.pos[1]
        if h < 0: return False 
        ...
        los_angle = np.arctan2(r_tm[1], r_tm[0]) 
        ...
        # ### CHANGE 2: THRUST LOGIC
        # If simulation time 't' is less than burn duration, Engine is ON
        if t < Config.burn_time:
            F_thrust = Config.thrust
        else:
            F_thrust = 0.0

        # D. TRANSFORMATIONS 
        gamma = np.arctan2(self.vel[1], self.vel[0]) 
        
        # ### CHANGE 3: ADDING THRUST TO THE EQUATION
        # Thrust acts WITH velocity (+), Drag acts AGAINST velocity (-)
        # Lift acts Perpendicular
        
        # Force along the velocity vector (Axial Force)
        F_axial = F_thrust - F_drag
        
        # Resolve into Inertial Frame (X, Y)
        # [Image of forces on a climbing aircraft]

        fx_total = F_axial * np.cos(gamma) - F_lift * np.sin(gamma)
        fy_total = F_axial * np.sin(gamma) + F_lift * np.cos(gamma)
        
        # E. INTEGRATION
        ax = fx_total / Config.mass
        ay = (fy_total / Config.mass) - Config.g 
...
def run_simulation():
    ...
    while t < config.max_time:
        # ### PASS 't' HERE ###
        flying = missile.update(t, config.dt, config.target_pos)
